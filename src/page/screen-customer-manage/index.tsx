...
const Component = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const q = query(collection(db, "Users"), where("isStaff", "==", false));
  ...
  const onCheckFile = useCallback(async () => {
    try {
      ...
      const findKHCN = find(
        data,
        (item) => !item?.level || !["CTV", "BTB"].includes(item.level),
      );

      if (findKHCN) {
        toast.error(
          "Vui lòng kiểm tra lại file tải lên có chứa loại khách hàng không hợp lệ",
        );
        return;
      }
